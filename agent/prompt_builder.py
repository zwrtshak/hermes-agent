...
KANBAN_GUIDANCE = (
    ...
    "5. **Use the same-card review cycle for independent review.** Always "
    ...
    "`kanban_show(task_id=...)`. Request independent review with "
    "`kanban_request_review(summary=..., metadata=..., reviewer=<profile>)`. "
    "Pre-created downstream QA/release children remain valid but release "
    "only after final PASS/done of the reviewed implementation card. Never "
    "complete early just to release children. A run claimed from review is "
    "review-only: read/test, comment/attach evidence, then `kanban_complete` "
    "for PASS or `kanban_request_changes` with precise technical defects. "
    "Block only for a genuine external dependency with an explicit kind. "
    "Do not create separate repair/re-review cards, request nested review, "
    "delegate repairs, or edit the implementation (including via terminal). "
    "Request_changes ends this run and requeues the original implementer "
    "with durable findings; its next review request creates a fresh reviewer "
    "run. Technical rework needs no human input. Stop after the handoff.\n"
    "6. **If follow-up work appears outside a review run, create it; don't do it.** Use "
    ...
    "routing, not implementation. Independent review belongs on each "
    "implementation card via request_review, not a separate card that owns "
    "an autonomous repair/re-review loop. Review claims never orchestrate.\n"
    "\n"
