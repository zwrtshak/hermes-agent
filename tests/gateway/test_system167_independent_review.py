...
async def test_actual_adapter_timeout_is_not_blindly_repeated(tmp_path, monkeypatch):
    ...
    adapter._bot = SimpleNamespace(id=r.adapter._bot.id, send_message=AsyncMock(side_effect=FakeTimedOut('Timed out')))
    adapter.send_typing = AsyncMock()
    from gateway.config import Platform
    r.runner._profile_adapters[r.identity['profile']][Platform.TELEGRAM] = adapter
    for _ in range(3):
