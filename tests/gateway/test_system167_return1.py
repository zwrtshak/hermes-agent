...
def bot_adapter(outcomes, bot_id=123):
    a=_make_adapter(); a._should_attempt_rich=lambda *a,**kw:False
    a._bot=SimpleNamespace(id=bot_id, send_message=AsyncMock(side_effect=outcomes)); a.send_typing=AsyncMock()
    return a
...
async def test_real_adapter_outcomes_survive_reload(tmp_path,monkeypatch,kind,expected,attempts):
    ...
    r.runner._profile_adapters[r.identity['profile']][Platform.TELEGRAM]=a
    for _ in range(5):
        r.guard=dc.Guard(r.guard.path)
        await r.runner._diggr_deliver(r.guard,r.identity)
        # Exercise the preserved retry/backoff window, not an expired deadline.
        r.clock[0]=max(r.clock[0], r.guard.get(r.task['task'])['deliveries'][0]['due'])
...
async def test_bound_local_pipeline(tmp_path,monkeypatch,profile,topic):
    ...
    session_tokens=r.runner._set_session_env(SessionContext(source=r.source,
    ...
    try:
        ...
        repeated=json.loads(dc.terminal_dispatch(dict(command='SYSTEM167_REGISTERED_WORKER',continuation_ticket=ticket),lambda args:pytest.fail('duplicate')))
        assert repeated['already_registered'] and repeated['visible_sent']
        assert repeated['task']==r.task['task']
    finally:
        r.runner._clear_session_env(session_tokens)
    ...
    adapters={p:bot_adapter([SimpleNamespace(message_id=88)], bot_id=123 if p=='diggr-main' else 456)
              for p in ['diggr-main','mira']}
    r.runner._profile_adapters={p:{Platform.TELEGRAM:a} for p,a in adapters.items()}
    await r.runner._diggr_deliver(r.guard,r.identity)
    ...
    assert sent.call_args.kwargs.get('message_thread_id') is None
...
async def test_restart_after_actual_send_claim_is_not_replayed(tmp_path,monkeypatch):
    import asyncio
    r=await setup(tmp_path,monkeypatch); result(r)
    a=bot_adapter([asyncio.CancelledError()])
    r.runner._profile_adapters[r.identity['profile']][Platform.TELEGRAM]=a
