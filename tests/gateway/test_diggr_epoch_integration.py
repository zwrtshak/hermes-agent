...
async def test_native_fifo_multiple_epochs_busy_and_control(tmp_path, monkeypatch):
    ...
    source = SessionSource(platform=Platform.TELEGRAM,profile='diggr-main',chat_id='fixture',user_id='fixture')
...
def test_real_cli_retry_receipt_on_temporary_home(tmp_path,monkeypatch):
    ...
    identity=dict(home=str(home),profile='diggr-main',session='fixture',session_key='fixture',
                  platform='telegram',chat_id='fixture',user_id='fixture',thread_id='')
    ...
    result=subprocess.run([sys.executable,'-m','hermes_cli.diggr_continuation','--home',str(home),
        'retry','--payload-json',json.dumps(payload)],capture_output=True,text=True,env=dict(os.environ,HOME=str(home)))
