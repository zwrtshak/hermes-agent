...
def test_native_transport_without_owner_grant_does_not_create_ledger(admission):
    from gateway.session_context import completion_launch_context
    guard, task, now = admission
    identity = task['identity']
    binding = dict(profile=identity['profile'], transport_profile=identity['profile'],
                   home_namespace=identity['home'], profile_home=identity['home'],
                   bot_id='fixture-bot', chat_type='dm', platform=identity['platform'],
    ...
    token = completion_launch_context.set((binding, lambda watcher: None))
    try:
        with pytest.raises(ValueError, match='confirmed native owner batch required'):
            guard.register(task, now=now)
    finally:
        completion_launch_context.reset(token)
    assert not guard.path.exists()
