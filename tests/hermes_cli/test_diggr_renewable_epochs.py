...
def registered(tmp_path, **overrides):
    ...
    guard.register(authorize(guard, task), now=-9)
...
class RenewableEpochTests(unittest.TestCase):
    ...
    def test_progress_epochs_and_no_progress_backoff(self):
        ...
        wake = guard.tick(identity, now=3200)
        guard.begin(identity, wake, now=3201)
        self.assertTrue(guard.ack(identity, wake, self.receipt(wake, '7'), 'main_live', 'validate', now=3202))
        row = guard.get('test')
        self.assertEqual(row['gate'], 'reconcile')
        self.assertGreater(row['recovery_attempt'], 0)
        self.assertGreater(row['due'], 3202)
        self.assertIsNone(guard.tick(identity, now=3202))
    ...
    def test_technical_retry_respects_reset_and_never_terminal(self):
        ...
        self.assertIsNone(guard.tick(identity, now=3591))
        self.assertEqual(guard.get('test')['status'], 'paused')
    ...
    def test_exited_legacy_pid_enters_readonly_reconciliation(self):
        guard, identity = registered(self.root)
        with guard.transaction() as tasks:
            tasks['test'].update(status='running', worker_pid=43210)
        with patch('psutil.pid_exists', return_value=False), patch.object(dc.time, 'time', return_value=2):
