...
def test_verified_idle_same_binding_recovers_without_migration(native_recovery, sent):
    r = native_recovery
    binding = dc.bind_visible_target(r.strategy['worker_route']['visible_target'])
    with r.guard.transaction() as rows:
        row = rows[r.task['task']]
        row['visible_binding'] = binding
        if sent:
            row['visible_sent'] = True
    r.evidence = report(r)
    before = r.row()
    dc.maintenance_control(request(r))
    after = r.row()
    assert after['visible_binding'] == binding
    assert after['action_id'] != before['action_id']
    assert after['effect_id'] != before['effect_id']
    assert after['policy']['recoveries'] == before['policy']['recoveries'] + 1
    assert after['policy']['deadline'] == before['policy']['deadline']
    dc.cmux_send.assert_not_called()



@pytest.mark.parametrize('hazard', ['live_child', 'unknown_child', 'busy_shell'])
def test_same_binding_recovery_keeps_process_safeguards(native_recovery, hazard):
    r = native_recovery
    binding = dc.bind_visible_target(r.strategy['worker_route']['visible_target'])
    with r.guard.transaction() as rows:
        row = rows[r.task['task']]
        row['visible_binding'] = binding
        row['visible_sent'] = True
        if hazard != 'busy_shell':
            row['worker_pid'] = 303
            row['worker_identity'] = dict(pid=303, start='prior-worker', executable='python')
        if hazard == 'live_child':
            row['child_identity'] = dict(pid=404, start='child', executable='codex')
            r.processes[404] = row['child_identity']
    if hazard == 'busy_shell':
...
def test_slow_recovery_observation_does_not_block_stop(native_recovery):
    import concurrent.futures
    import contextvars
    import threading
    r = native_recovery
    entered, release = threading.Event(), threading.Event()
    original = dc.verify_recovery_worktree
    def slow(*args):
        entered.set()
        assert release.wait(5)
        return original(*args)
    r.monkeypatch.setattr(dc, 'verify_recovery_worktree', slow)
    context = contextvars.copy_context()
    with concurrent.futures.ThreadPoolExecutor(2) as pool:
        recovery = pool.submit(context.run, dc.maintenance_control, request(r))
        try:
            assert entered.wait(5)
            stop = pool.submit(r.guard.control, r.identity, 'cancelled')
            stop.result(timeout=5)  # Must finish while external inspection is held.
...
@pytest.mark.parametrize('change', ['row', 'grant', 'evidence'])
def test_recovery_revalidates_after_external_observation(native_recovery, change):
    r = native_recovery
    original = dc.verify_recovery_worktree
    def changed(*args):
        original(*args)
        if change == 'evidence':
            Path(r.evidence['path']).write_text('{}')
        else:
            with r.guard.transaction() as rows:
                if change == 'row':
                    rows[r.task['task']]['policy']['recoveries'] += 1
                else:
                    batch = rows[r.task['task']]['policy']['batch']
                    rows.grants['batches'][batch]['revoked'] = True
    r.monkeypatch.setattr(dc, 'verify_recovery_worktree', changed)
    with pytest.raises(ValueError):
        dc.maintenance_control(request(r))
    assert not Path(r.strategy['artifact'] + '.reservation').exists()
