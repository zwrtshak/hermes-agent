...
def crash_after_claim(path, identity, wake):
    assert Guard(path).begin(identity, wake, now=21)
    os._exit(7)
...
class GuardTests(unittest.TestCase):
    def setUp(self):
        ...
        self.g.register(self.task, now=1)
    ...
    def test_actual_process_exit_after_claim_requires_reconciliation(self):
        self.completed()
        wake = self.g.tick(self.identity, now=20)
        proc = multiprocessing.get_context('fork').Process(target=crash_after_claim,
                  args=(str(self.path), self.identity, wake))
        proc.start()
        proc.join(5)
        self.assertEqual(proc.exitcode, 7)
        self.assertEqual(self.g.get(self.task['task'])['status'], 'executing')
    ...
    def test_concurrent_process_ticks_one_wake(self):
        self.completed()
        with concurrent.futures.ProcessPoolExecutor(4, mp_context=multiprocessing.get_context('fork')) as pool:
